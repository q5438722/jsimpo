...
const elHasClippableOverflow = function ($el) {
  return OVERFLOW_PROPS.includes($el.css('overflow')) || OVERFLOW_PROPS.includes($el.css('overflow-y')) || OVERFLOW_PROPS.includes($el.css('overflow-x'));
};
...
const elAtCenterPoint = function ($el) {
  ...
  var { topCenter, leftCenter } = elProps.fromElViewport;
...
const elIsNotElementFromPoint = function ($el) {
  ...
  if (($el.css('pointer-events') === 'none' || $el.parent().css('pointer-events') === 'none') && $elAtPoint && $elements.isAncestor($el, $elAtPoint)) {
    return false;
  }
