  ...
  fs.readdir(pluginsDirPath, (_, files) => {
    files.forEach(file => {
      if (pluginRegEx.test(file)) {
        var entryPath = path.resolve(__dirname, '../packages', '@vue', file, 'README.md');
        var entryContent = fs.readFileSync(entryPath);
        var docPath = path.resolve(__dirname, '../docs', 'core-plugins', `${file.replace('cli-plugin-', '')}.md`);
        fs.writeFile(docPath, entryContent, () => {});
      }
    });
  });
