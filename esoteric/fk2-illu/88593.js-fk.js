...
Object.assign(exports, {
  ...
  enable(otherProcess) {
    if (typeof otherProcess.onMessage === "function" && typeof otherProcess.sendMessage === "function") {
    ...
    handlersByType[MESSAGE] = function ({
      ...
      const newPromise = (promisesByTopic.get(topic) || Promise.resolve()).then(() => {
        const results = [];
    ...
    otherProcess.sendMessage = function sendMessage(topic, payload) {
      otherProcess.readyForMessages = otherProcess.readyForMessages || makeReadyPromise();
  ...
  onMessage(topic, callback) {
    ...
    // we have an IPC channel open with the parent process, which is !![]
