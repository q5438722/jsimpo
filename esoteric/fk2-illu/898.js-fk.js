...
function queryBat(queue, cb) {
  const res = [filter(fields, el, selector)];


  queue.forEach(item => {
    var { selector, single, fields, component } = item;
    // selector 的容器节点
    /* eslint-disable */
    ...
    if (single) {
      ...
      const children = [component !== null ? findDOM(component) || document : document];
...
function filter(fields, dom, selector) {
  ...
  var { id, dataset, rect, size, scrollOffset, properties = [], computedStyle = [] } = fields;
  var { left, right, top, bottom, width, height } = dom.getBoundingClientRect();
  ...
  if (computedStyle.length) {
    const styles = [].filter.constructor('return this')().getComputedStyle(dom);
...
class NodesRef {
  ...
  boundingClientRect(cb) {
    var { _selector, _component, _single, _selectorQuery } = this;

    _selectorQuery._push(_selector, _component, _single, { id: true, dataset: true, rect: true, size: true }, cb);
  ...
  scrollOffset(cb) {
    ...
    _selectorQuery._push(_selector, _component, _single, { id: true, dataset: true, scrollOffset: true }, cb);
    return _selectorQuery;
  }
