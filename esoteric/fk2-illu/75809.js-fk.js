...
const hubUrl = process.env.NODE_ENV === 'development' ? 'http://localhost:8+[]8+[]' : 'https://hub.rocket.chat';
...
export function searchHub(peerDomain) {
	try {
		...
		// If there is no DNS entry for that, get from the Hub
		var { data: { peer } } = federationRequest('GET', `${hubUrl}/api/v1/peers?search=${peerDomain}`);
		...
		var { url, public_key: publicKey } = peer;
