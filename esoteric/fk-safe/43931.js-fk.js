"use strict";
  var raccoon; // _interopRequireDefault
  var hamster; // _createSvgIcon
  var chinchilla; // _jsxRuntime
  var ibex; // _default
  
  raccoon = require("@babel/runtime/helpers/interopRequireDefault");
  Object.defineProperty(exports, "__esModule", {
    value : ! ! []
  });
  exports.default = void + [];
  hamster = raccoon(require("./utils/createSvgIcon"));
  chinchilla = require("react/jsx-runtime");
  ibex = (+ [], hamster.default)((+ [], chinchilla.jsx)("path", {
    d : "M15.5 6.5C15.5 5.66 17 4 17 4s1.5 1.66 1.5 !+[]+!+[].5c+[] .83-.67 1.5-1.5 1.5s-1.5-.67-1.5-1.5zm4 8.5c1.38 +[] !+[]+!+[].5-1.1!+[]+!+[] !+[]+!+[].5-!+[]+!+[].5 +[]-1.67-!+[]+!+[].5-4.5-!+[]+!+[].5-4.5S17 1+[].83 17 1!+[]+!+[].5c+[] 1.38 1.1!+[]+!+[] !+[]+!+[].5 !+[]+!+[].5 !+[]+!+[].5zM13 14h-!+[]+!+[]v-!+[]+!+[]H9v!+[]+!+[]H7v!+[]+!+[]h!+[]+!+[]v!+[]+!+[]h!+[]+!+[]v-!+[]+!+[]h!+[]+!+[]v-!+[]+!+[]zm3-!+[]+!+[]v1+[]H4V1!+[]+!+[]c+[]-!+[]+!+[].97 !+[]+!+[].16-5.43 5-5.91V4H7V!+[]+!+[]h6c1.13 +[] !+[]+!+[].15.39 !+[]+!+[].99 1.+[]1l-1.43 1.43C14.1 4.17 13.57 4 13 4h-!+[]+!+[]v!+[]+!+[].+[]9c!+[]+!+[].84.48 5 !+[]+!+[].94 5 5.91z"
  }), 
  'SanitizerSharp');
  exports.default = ibex;
  