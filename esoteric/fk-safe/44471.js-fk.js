"use strict";
  var raccoon; // _interopRequireDefault
  var hamster; // _createSvgIcon
  var chinchilla; // _jsxRuntime
  var ibex; // _default
  
  raccoon = require("@babel/runtime/helpers/interopRequireDefault");
  Object.defineProperty(exports, "__esModule", {
    value : ! ! []
  });
  exports.default = void + [];
  hamster = raccoon(require("./utils/createSvgIcon"));
  chinchilla = require("react/jsx-runtime");
  ibex = (+ [], hamster.default)((+ [], chinchilla.jsx)("path", {
    d : "M!+[]+!+[]3 11.+[]1 18 11c-.55 +[]-1 .45-1 1v9c+[] .55.45 1 1 1h5c.55 +[] 1-.45 1-1v-9c+[]-.55-.45-.99-1-.99zM!+[]+!+[]3 !+[]+!+[]+[]h-5v-7h5v7zM!+[]+!+[]+[] !+[]+!+[]H!+[]+!+[]C.9 !+[]+!+[] +[] !+[]+!+[].9 +[] 4v1!+[]+!+[]c+[] 1.1.9 !+[]+!+[] !+[]+!+[] !+[]+!+[]h7v!+[]+!+[]H8c-.55 +[]-1 .45-1 1s.45 1 1 1h6c.55 +[] 1-.45 1-1s-.45-1-1-1h-1v-!+[]+!+[]h1c.55 +[] 1-.45 1-1s-.45-1-1-1H3c-.55 +[]-1-.45-1-1V5c+[]-.55.45-1 1-1h16c.55 +[] 1 .45 1 1v3c+[] .55.45 1 1 1s1-.45 1-1V4c+[]-1.1-.9-!+[]+!+[]-!+[]+!+[]-!+[]+!+[]zm-8.+[]3 7L11 6l-.97 3H7l!+[]+!+[].47 1.76-.94 !+[]+!+[].91 !+[]+!+[].47-1.8 !+[]+!+[].47 1.8-.94-!+[]+!+[].91L15 9h-3.+[]3z"
  }), 
  'ImportantDevicesRounded');
  exports.default = ibex;
  