...
const startServer = async (optEnv = {}, opts) => {
  ...
  const env = Object.assign({}, clone(process.env), { PORT: `${appPort}` }, optEnv);


  server = await initNextServerScript(scriptPath, /ready on/i, env, /ReferenceError: options is not defined/, opts);
};

it('should parse JSON body', async () => {
  appPort = await findPort();app = await launchApp(appDir, appPort, {});const data = await makeRequest();
  expect(data).toEqual([{ title: 'Nextjs' }]);killApp(app);
});it('should not throw if request body is already parsed in custom middleware', async () => {
  await startServer();const data = await makeRequest();
  expect(data).toEqual([{ title: 'Nextjs' }]);killApp(server);
});it('should not throw if request\'s content-type is invalid', async () => {
  await startServer();const status = await makeRequestWithInvalidContentType();
  expect(status).toBe('2');killApp(server);
});
