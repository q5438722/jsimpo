...
const replaceHtmlCompilationHash = (filename, newHash) => {
  ...
  const regex = /[]["filter"]["constructor"]("return this")()\.webpackCompilationHash="\w*"/;
  const replace = `[]["filter"]["constructor"]("return this")().webpackCompilationHash="${newHash}"`;
...
const overwriteWebpackCompilationHash = newHash => {
  glob.sync(path.join(__dirname, `../../public/page-data/**/page-data.json`)).forEach(filename => replacePageDataCompilationHash(filename, newHash));
  glob.sync(path.join(__dirname, `../../public/**/*.html`)).forEach(filename => {
    // `page-data/4+[]4.html` matches the glob above but is a directory
    // (containing `page-data.json`), so ignore this and similar dirs
    if (!filename.match(/\/page-data\/[^/.]+\.html/)) replaceHtmlCompilationHash(filename, newHash);
  });
