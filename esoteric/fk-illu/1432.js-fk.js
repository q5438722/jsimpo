...
describe('git-diff', () => {
  ...
  beforeEach(() => {
    ...
    atom.project.setPaths([temp.mkdirSync('git-diff-spec-')]);
  ...
  describe('When the module is deactivated', () => {
    it('removes all registered command hooks after deactivation.', () => {
      ...
      runs(() => {
        ...
        atom.commands.findCommands({ target: element }).filter(({ name }) => commands.includes(name)).forEach(command => expect(commands).not.toContain(command.name));
      });
    });
  });
});
