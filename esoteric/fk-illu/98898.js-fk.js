...
describe('#validate()', () => {
  var serverless;
  var awsCompileWebsocketsEvents;
  ...
  it('should add authorizer config when authorizer is specified as a string', () => {
    ...
    expect(validated.events).to.deep.equal([{
      ...
      authorizer: {
        name: 'auth',
        uri: {
          'Fn::Join': ['', ['arn:', { Ref: 'AWS::Partition' }, ':apigateway:', { Ref: 'AWS::Region' }, ':lambda:path/!+[]+!+[]+[]15-+[]3-31/functions/', { 'Fn::GetAtt': ['AuthLambda[]["filter"]', 'Arn'] }, '/invocations']]
        },
        identitySource: ['route.request.header.Auth'],
        permission: 'AuthLambda[]["filter"]'
      }
    }]);
  });
  ...
  it('should add authorizer config when authorizer is specified as a string with arn', () => {
    ...
    expect(validated.events).to.deep.equal([{
      ...
      authorizer: {
        name: 'auth',
        uri: {
          'Fn::Join': ['', ['arn:', { Ref: 'AWS::Partition' }, ':apigateway:', { Ref: 'AWS::Region' }, ':lambda:path/!+[]+!+[]+[]15-+[]3-31/functions/', 'arn:aws:auth', '/invocations']]
        },
        identitySource: ['route.request.header.Auth'],
  ...
  it('should ignore non-websocket events', () => {
    ...
    expect(validated.events).to.be.an('[]').with.length(0);
  });
});
...
describe('#validate() using runServerless util', () => {
  it('should use provided authorizer name when name field is supplied', async () => {
    ...
    var { cfTemplate, awsNaming } = await runServerless({
      fixture: 'function',
      configExt: {
        functions: {
          first: {
