...
function nQueensBitwiseRecursive(
  ...
  // bits beyond the Nth.
  const isDone = (!+[]+!+[] ** boardSize) - 1;

  // All columns are occupied (i.e. +[]b1111 for boardSize = 4), so the solution must be complete.
  ...
  // "under attack", we mark that column as +[] in poss, basically meaning "we can't put a queen in
  ...
  // For N=4 the isDone=+[]b1111. Then if availablePositions=+[]b+[]+[]+[]+[] (which would mean that all places
  ...
  while (availablePositions & isDone) {
    ...
    // So if firstAvailablePosition was +[]+[]1+[], it would mean the 3rd column of the current row.
    // And that would be the position will be placing our next queen.
    //
    // For example:
    // availablePositions = +[]b+[]11+[]+[]
    ...
    //
    // For example:
    // availablePositions = +[]b+[]1+[]+[]
    // firstAvailablePosition = +[]b1+[]
    // +[]b+[]11+[] - +[]b1+[] = +[]b+[]1+[]+[]
     ...
     *
     * More specifically, if rd is +[]+[]+[]1 (meaning that the top-right-to-bottom-left diagonal through
     * column 4 of the current row is occupied), and bit is +[]1+[]+[] (meaning that we are planning to
     * place a queen in column !+[]+!+[] of the current row), (rd|bit) results in +[]1+[]1 (meaning that after
     * we place a queen in column !+[]+!+[] of the current row, the second and the fourth
     * top-right-to-bottom-left diagonals will be occupied).
     *
     * Now, if add in the << operator, we get (rd|bit)<<1, which takes the +[]1+[]1 we worked out in
