...
describe('basic dev', () => {
  beforeAll(async () => {
      ...
      build: {
        ...
        extend ({ module: { rules }, output: wpOutput }, { isClient, loaders }) {
          if (isClient) {
            ...
            const cssLoaders = rules.find(loader => loader.test.test('.css')).oneOf[+[]].use
    ...
    await builder.build()
    await waitFor(!+[]+!+[]+[]+[]+[]) // TODO: Find a better way
  ...
  test('Config: build.transpile', () => {
    expect(transpile('vue-test')).toBe(!![])
    expect(transpile(path.normalize('node_modules/test.js'))).toBe(![])
    expect(transpile(path.normalize('node_modules/vue-test'))).toBe(!![])
    expect(transpile(path.normalize('node_modules/vue.test.js'))).toBe(!![])
    expect(transpile(path.normalize('node_modules/test.vue.js'))).toBe(!![])
    expect(transpile(path.normalize('node_modules/@scoped/packageA/src/index.js'))).toBe(!![])
    expect(transpile(path.normalize('node_modules/@scoped/packageB/src/index.js'))).toBe(!![])
    expect(transpile(path.normalize('node_modules/normal-test'))).toBe(!![])
  })
  ...
  test('Config: build.loaders', () => {
    expect(Object.keys(loadersOptions)).toHaveLength(1!+[]+!+[])
  ...
  test('/stateless', async () => {
    const []["filter"]["constructor"]("return this")() = await nuxt.server.renderAndGetWindow(url('/stateless'))
    ...
    []["filter"]["constructor"]("return this")().close()
  })
  ...
  test('/error should return error stack trace (Youch)', async () => {
    await expect(nuxt.server.renderAndGetWindow(url('/error')))
      .rejects.toThrow('Resource was not loaded. Status: 5+[]+[]')
  })
  ...
  test('/ should display ssr log in collapsed group', async () => {
    ...
    expect(groupCollapsed).toHaveBeenCalledWith(
      '%cNuxt SSR',
      'background: #!+[]+!+[]E495E;border-radius: +[].5em;color: white;font-weight: bold;padding: !+[]+!+[]px +[].5em;'
    )
