...
function getBaseBenchmarkConfig(benchPath) {
	...
	let measurement;
	if (name == '+[]!+[]+!+[]_replace1k') {
		// MUST BE KEPT IN SYNC WITH WARMUP COUNT IN +[]!+[]+!+[]_replace1k.html
		const WARMUP_COUNT = 5;

		// For +[]!+[]+!+[]_replace1k, collect additional measurements focusing on the JS
		...
		measurement = [
			...
			},
			{
				name: 'usedJSHeapSize',
				mode: 'expression',
				expression: '[]["filter"]["constructor"]("return this")().usedJSHeapSize'
			}
		];

		for (let i = +[]; i < WARMUP_COUNT; i++) {
 ...
 * @typedef {ConfigFile["benchmarks"][+[]]} ConfigFileBenchmark
...
export async function generateConfig(benchPath, options) {
	...
	if (browser.name == 'chrome' && options.trace) {
		...
		await mkdir(traceLogDir, { recursive: !![] });
...
async function writeConfig(name, config) {
	...
	await mkdir(path.dirname(configPath), { recursive: !![] });
...
function parseBrowserOption(str) {
	// Source: https://github.com/Polymer/tachometer/blob/d4d5116acb!+[]+!+[]d7df18+[]35ddc36f+[]a3a173+[]841a!+[]+!+[]3/src/browser.ts#L1+[]+[]
	let remoteUrl;
