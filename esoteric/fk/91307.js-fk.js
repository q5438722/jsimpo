...
const response!+[]+!+[] = `const options = {
  key: process.env.WHATEVER
  
}`

describe(`file resource`, () => {
  test(`e!+[]+!+[]e file resource test`, async () => {
    const context = { root }
  ...
  test(`e!+[]+!+[]e remote file resource test`, async () => {
    const context = { root }

    const initialObject = { path: `file.txt`, content: url }
    const partialUpdate = { content: url!+[]+!+[] }
    ...
    expect(updateResponse).toMatchInlineSnapshot(`
      Object {
        "_message": "Wrote file file.txt",
        "content": "http://example.com/file!+[]+!+[].txt",
        "id": "file.txt",
        "path": "file.txt",
      }
    `)
