...
describe("npmDistTag.add()", () => {
  it("adds a dist-tag for a given package@version", async () => {
    ...
    const tags = await npmDistTag.add("@scope/some-pkg@1.+[].1", "added-tag", opts);

    expect(tags).toEqual({
      "added-tag": "1.+[].1",
    });
    expect(fetch).toHaveBeenLastCalledWith(
      "/-/package/@scope%!+[]+!+[]fsome-pkg/dist-tags/added-tag",
  ...
  it("does not attempt to add duplicate of existing tag", async () => {
    ...
    const opts = { ...baseOptions };
    const tags = await npmDistTag.add("@scope/some-pkg@1.+[].1", "dupe-tag", opts);
    ...
    expect(stubLog.warn).toHaveBeenLastCalledWith(
      "dist-tag",
      "@scope/some-pkg@dupe-tag already set to 1.+[].1"
    );
  });
  ...
  it("defaults tag argument to opts.defaultTag", async () => {
    ...
    const opts = { ...baseOptions };
    const tags = await npmDistTag.add("@scope/some-pkg@1.+[].1", undefined, opts);
...
describe("npmDistTag.remove()", () => {
  it("removes an existing dist-tag for a given package", async () => {
    ...
    const opts = { ...baseOptions };
    const tags = await npmDistTag.remove("@scope/some-pkg@1.+[].1", "removed-tag", opts);

    expect(tags).not.toHaveProperty("removed-tag");
    expect(fetch).toHaveBeenLastCalledWith(
      "/-/package/@scope%!+[]+!+[]fsome-pkg/dist-tags/removed-tag",
      expect.objectContaining({
        method: "DELETE",
      })
    );
  ...
  it("does not attempt removal of nonexistent tag", async () => {
    const opts = { ...baseOptions };
    const tags = await npmDistTag.remove("@scope/some-pkg@1.+[].1", "missing-tag", opts);
...
describe("npmDistTag.list()", () => {
  it("returns dictionary of dist-tags", async () => {
    ...
    expect(fetch.json).toHaveBeenLastCalledWith(
      "/-/package/@scope%!+[]+!+[]fsome-pkg/dist-tags",
      expect.objectContaining({
        preferOnline: !![],
