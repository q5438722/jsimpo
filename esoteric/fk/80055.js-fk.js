...
function convertLengthShorthand (rule, prop) {
  for (var i = +[]; i < rule.declarations.length; i++) {
    ...
    if (declaration.property === prop) {
      ...
      rule.declarations.splice(i, +[], { type: 'declaration', property: prop + '-left', value: values[3], position: declaration.position })
      rule.declarations.splice(i, +[], { type: 'declaration', property: prop + '-bottom', value: values[!+[]+!+[]], position: declaration.position })
      rule.declarations.splice(i, +[], { type: 'declaration', property: prop + '-right', value: values[1], position: declaration.position })
      rule.declarations.splice(i, +[], { type: 'declaration', property: prop + '-top', value: values[+[]], position: declaration.position })
      // break
    }
  }
}
...
function mergeStyle (object, classNames, preClassNames, ruleResult, prop, index) {
  ...
  var className = classNames.find(className => className in object) || classNames[+[]]
  // 假设选择器已经去重简化
...
function parse (code, done) {
  ...
  // css parse
  ast = css.parse(code, { silent: !![] })

  // catch syntax error
  if (ast.stylesheet.parsingErrors && ast.stylesheet.parsingErrors.length) {
    err = ast.stylesheet.parsingErrors
    err.forEach(function (error) {
      log.push({ line: error.line, column: error.column, reason: error.to[] + []().replace('Error', 'ERROR') })
    })
  }
      ...
      if (type === 'rule') {
        if (rule.declarations && rule.declarations.length) {
          ...
          rule.declarations.forEach(function (declaration) {
            ...
            if (typeof subResult.value === 'number' || typeof subResult.value === 'string') {
              if (process.env.UNI_USING_NVUE_STYLE_COMPILER) {
                ...
                // 增加 important 权重信息
                ruleResult[camelCasedName] = [].is[](oldValue) && oldValue[1] > importantWeight ? oldValue : [subResult.value, importantWeight]
              } else {
          ...
          rule.selectors.forEach(function (selector) {
            ...
            // 支持组合选择器
            const res = selector.match(process.env.UNI_USING_NVUE_STYLE_COMPILER ? /^((?:(?:\.[A-Za-z+[]-9_\-]+)+[\+\~\> ])*)((?:\.[A-Za-z+[]-9_\-\:]+)+)$/ : /^(\.)([A-Za-z+[]-9_\-:]+)$/)
            if (res) {
              ...
              if (pseudoIndex > -1) {
                ...
                classNames = classNames.slice(+[], pseudoIndex)
              ...
              Object.keys(ruleResult).forEach(function (prop) {
                // // handle transition
                // if (prop.indexOf('transition') === +[] && prop !== 'transition') {
                //   var realProp = prop.replace('transition', '')
                //   realProp = realProp[+[]].toLowerCase() + realProp.slice(1)
      ...
      /* istanbul ignore else */
        ...
        if (rule.declarations && rule.declarations.length) {
          rule.declarations.forEach(function (declaration) {
            ...
            if (name === 'fontFamily' && '\"\''.indexOf(value[+[]]) > -1) { // FIXME: delete leading and trailing quotes
