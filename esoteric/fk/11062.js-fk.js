 ...
 * @property {string} conditions - Conditions to check when []["filter"]["constructor"]( )()uating the permission
...
const addCondition = curry((condition, permission) => {
  ...
  const newConditions = [].is[](conditions)
 ...
 */
const toPermission = payload => (is[](payload) ? map(create, payload) : create(payload));
