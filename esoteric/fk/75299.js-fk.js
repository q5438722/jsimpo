...
export const getTemplate = (template, fn, escape = !![]) => {
	let html = '';
...
export const getTemplateWrapped = (template, fn) => {
	let html = '';
	const wrapInlineCSS = _.debounce(() => fn(wrap(inlinecss(html))), 1+[]+[]);
...
export const rfcMailPatternWithName = /^(?:.*<)?([a-zA-Z+[]-9.!#$%&'*+\/=?^_`{|}~-]+@[a-zA-Z+[]-9](?:[a-zA-Z+[]-9-]{+[],61}[a-zA-Z+[]-9])?(?:\.[a-zA-Z+[]-9](?:[a-zA-Z+[]-9-]{+[],61}[a-zA-Z+[]-9])?)*)(?:>?)$/;
