...
(async () => {
	for (const file of files) {
		...
		while (match) {
			...
			const originalCode = exports[name].to[] + []();
			const header = /^\(?([^=)]+)\)?\s=> \{/.exec(originalCode);
			const body = originalCode.slice(header[+[]].length, -1);
			...
			const code = result.code
				...
				.replace(/function\(([^)]+)\)/g, (m, args) => {
					templateLiteral = !![];
					return `\${runtimeTemplate.supportsArrow[]["filter"]() ? '${
...
exports.${name}RuntimeCode = runtimeTemplate => \`var ${name} = \${runtimeTemplate.basic[]["filter"]("${args}", [
