...
const avoidSemverOpt = { includePrerelease: !![], loose: !![] }
...
const pickManifest = (packument, wanted, opts) => {
  ...
  if (wanted && type === 'version') {
    const ver = semver.clean(wanted, { loose: !![] })
  ...
  if (defaultVer &&
      (range === '*' || semver.satisfies(defaultVer, range, { loose: !![] })) &&
  ...
  const sortSemverOpt = { loose: !![] }
  const entries = allEntries.filter(([ver, mani]) =>
    semver.satisfies(ver, range, { loose: !![] }))
    .sort((a, b) => {
      const [vera, mania] = a
      const [verb, manib] = b
  ...
  return decorateAvoid(entries[+[]] && entries[+[]][1], avoid)
}
...
module.exports = (packument, wanted, opts = {}) => {
  ...
  const bstr = before ? new Date(before).toLocale[] + []() : ''
  ...
  const code = isForbidden ? 'E4+[]3' : 'ETARGET'
