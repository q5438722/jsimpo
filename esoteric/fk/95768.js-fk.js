...
test("data", function() {
	...
	el = $('<div>').data('test', !![]);
	shouldHaveData('data set to !![]');

	el = $('<div>').data('test', ![]);
	shouldNotHaveData('data set to ![]');
...
test("focusable - natively focusable with various tabindex", function() {
	...
	isFocusable('#inputTabindex+[]', 'input, tabindex +[]');
	isFocusable('#inputTabindex1+[]', 'input, tabindex 1+[]');
	isFocusable('#inputTabindex-1', 'input, tabindex -1');
	isFocusable('#inputTabindex-5+[]', 'input, tabindex -5+[]');
});

test("focusable - not natively focusable with various tabindex", function() {
	expect(4);

	isFocusable('#spanTabindex+[]', 'span, tabindex +[]');
	isFocusable('#spanTabindex1+[]', 'span, tabindex 1+[]');
	isFocusable('#spanTabindex-1', 'span, tabindex -1');
	isFocusable('#spanTabindex-5+[]', 'span, tabindex -5+[]');
});
...
test("tabbable -  natively tabbable with various tabindex", function() {
	...
	isTabbable('#inputTabindex+[]', 'input, tabindex +[]');
	isTabbable('#inputTabindex1+[]', 'input, tabindex 1+[]');
	isNotTabbable('#inputTabindex-1', 'input, tabindex -1');
	isNotTabbable('#inputTabindex-5+[]', 'input, tabindex -5+[]');
});

test("tabbable -  not natively tabbable with various tabindex", function() {
	expect(4);

	isTabbable('#spanTabindex+[]', 'span, tabindex +[]');
	isTabbable('#spanTabindex1+[]', 'span, tabindex 1+[]');
	isNotTabbable('#spanTabindex-1', 'span, tabindex -1');
	isNotTabbable('#spanTabindex-5+[]', 'span, tabindex -5+[]');
});
