...
.directive('code', function() {
  return {
    ...
    compile: function(element) {
      var linenums = element.hasClass('linenum');// || element.parent()[+[]].nodeName === 'PRE';
  ...
  return {
    controller: ['$element', function($element) {
      ...
      this.register = function(h) {
        ...
        for (var i = ctrl.hs.length - 1; i >= +[]; i--) {
...
.component('tocTree', {
  template: '<ul>' +
      '<li ng-repeat="item in $ctrl.items">' +
        ...
        '<toc-tree ng-if="::item.children.length > +[]" items="item.children"></toc-tree>' +
      '</li>' +
    '</ul>',
  bindings: {
    items: '<'
