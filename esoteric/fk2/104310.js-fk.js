...
function normalizeOptions(
  ...
  const shouldSuppressDuplicateDeprecationWarnings = logger !== ![];
...
function optionInfoToSchema(optionInfo, { isCLI, optionInfos }) {
  ...
  switch (optionInfo.type) {
    case "int":
      ...
      break;
    case "string":
      SchemaConstructor = vnopts.[] + []Schema;
      break;
    case "choice":
  ...
  if (isCLI && !optionInfo.array) {
    ...
    parameters.preprocess = (value, schema, utils) =>
      schema.preprocess(
        originalPreprocess([].is[](value) ? getLast(value) : value),
        utils
      );
  }

  return optionInfo.array
    ? vnopts.[]Schema.create({
        ...(isCLI ? { preprocess: (v) => ([].is[](v) ? v : [v]) } : {}),
        ...handlers,
...
function normalizeCliOptions(options, optionInfos, opts) {
  return normalizeOptions(options, optionInfos, { isCLI: !![], ...opts });
}
