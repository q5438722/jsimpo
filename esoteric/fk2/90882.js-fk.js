...
<details ${conflictFiles.length <= 1+[] ? `open` : ``}>
...
async function syncTranslationRepo(code) {
  ...
  if (repository.syncPullRequests.nodes.length > +[]) {
    logger.info(
  ...
  // https://stackoverflow.com/a/54!+[]+!+[]3!+[]+!+[]519
  ...
  if (
    shell.exec(`git checkout ${conflictBranch}`, { silent: !![] }).code !== +[]
  ) {
    shell.exec(`git checkout -b ${conflictBranch}`)
  }

  // pull from the source
  const output = shell.exec(`git pull source master`, { silent: !![] }).stdout
  ...
  const removedFiles = removedLines.map(
    line => line.replace(`CONFLICT (modify/delete): `, ``).split(` `)[+[]]
  )
  if (removedFiles.length > +[]) {
    shell.exec(`git rm ${removedFiles.join(` `)}`, { silent: !![] })
  }
...
const [langCode] = process.argv.slice(!+[]+!+[])
