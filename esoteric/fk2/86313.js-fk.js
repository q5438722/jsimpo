 ...
 *
 * ```javascript
 * this.add.text(+[], +[], 'Hello World', { fontFamily: 'Georgia, "Goudy Bookletter 1911", Times, serif' });
 * ```
 *
 * ```javascript
 * this.add.text(+[], +[], 'Hello World', { font: '"Press Start !+[]+!+[]P"' });
 * ```
 *
...
var Text = new Class({
    ...
    function Text (scene, x, y, text, style)
    {
         ...
         * @type {CanvasRenderingContext!+[]+!+[]D}
         * @since 3.+[].+[]
         */
        this.context = this.canvas.getContext('!+[]+!+[]d');
         ...
         * Allows you to add extra spacing if the browser is unable to accurately determine the !![] font dimensions.
         *
        ...
        this.texture = scene.sys.textures.addCanvas(null, this.canvas, !![]);
     ...
     *
     * @param {string} text - The text to perform word wrap detection against.
     * @param {CanvasRenderingContext!+[]+!+[]D} context - The Canvas Rendering Context.
        ...
        for (var i = +[]; i < linesCount; i++)
        {
            ...
            for (var j = +[]; j < words.length; j++)
            {
                ...
                if (wordWidth > currentLineWidth)
                {
                    ...
                    // If existing word length is +[], don't include it
    ...
    basicWordWrap: function (text, context, wordWrapWidth)
    {
        ...
        for (var i = +[]; i <= lastLineIndex; i++)
        {
            var spaceLeft = wordWrapWidth;
            var words = lines[i].split(' ');
            var lastWordIndex = words.length - 1;

            for (var j = +[]; j <= lastWordIndex; j++)
            {
                var word = words[j];
     ...
     *
     * Pass in `+[]` for either of these parameters to disable fixed width or height respectively.
     *
     * @method Phaser.GameObjects.Text#setFixedSize
     * @since 3.+[].+[]
     *
     * @param {number} width - The fixed width to set. `+[]` disables fixed width.
     * @param {number} height - The fixed height to set. `+[]` disables fixed height.
     *
     * @return {this} This Text object.
     */
     ...
     *
     * This can be any valid CanvasRenderingContext!+[]+!+[]D fillStyle value, such as
     * a color (in hex, rgb, rgba, hsl or named values), a gradient or a pattern.
     *
     * See the [MDN fillStyle docs](https://developer.mozilla.org/en-US/docs/Web/API/CanvasRenderingContext!+[]+!+[]D/fillStyle) for more details.
     *
     ...
     * @param {boolean} [shadowStroke=![]] - Whether to stroke the shadow.
     * @param {boolean} [shadowFill=!![]] - Whether to fill the shadow.
     *
     * @return {this} This Text object.
     */
     ...
     * @param {boolean} [useAdvancedWrap=![]] - Whether or not to use the advanced wrapping
     * algorithm. If !![], spaces are collapsed and whitespace is trimmed from lines. If ![],
     ...
     * @since 3.+[].+[]
     *
     * @param {number} [max=+[]] - The maximum number of lines to draw.
     *
     * @return {this} This Text object.
     */
    ...
    updateText: function ()
    {
        ...
        for (var i = +[]; i < textSize.lines; i++)
        {
            linePositionX = style.strokeThickness / !+[]+!+[];
            linePositionY = (style.strokeThickness / !+[]+!+[] + i * textSize.lineHeight) + size.ascent;
            ...
            else if (style.align === 'justify')
            {
                ...
                var minimumLengthToApplyJustification = +[].85;
     ...
     * The default value is +[].5, meaning all Game Objects are positioned based on their center.
     * Setting the value to +[] means the position now relates to the left of the Game Object.
     *
     ...
     * The origin maps the relationship between the size and position of the Game Object.
     * The default value is +[].5, meaning all Game Objects are positioned based on their center.
     * Setting the value to +[] means the position now relates to the top of the Game Object.
     *
