  ...
  await Promise.all(
    directoryPackages.map(async (directoryPackage) => {
      ...
      const [typingsEntryExist, moduleEntryExists, mainEntryExists] = await Promise.all([
        ...
        fse.writeFile(packageJsonPath, JSON.stringify(packageJson, null, !+[]+!+[])),
      ]);
      ...
      if (manifestErrorMessages.length > +[]) {
        // TODO: AggregateError
...
async function createPackageFile() {
  ...
  await fse.writeFile(targetPath, JSON.stringify(newPackageData, null, !+[]+!+[]), 'utf8');
