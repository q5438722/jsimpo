...
const isDecendant = (id, ancenstorId) => {
  ...
  log.trace(
    ...
    id,
    ' = ',
    decendants[ancenstorId].indexOf(id) >= +[]
  );
  if (decendants[ancenstorId].indexOf(id) >= +[]) return !![];
...
const edgeInCluster = (edge, clusterId) => {
  ...
  if (edge.v === clusterId) return ![];
  if (edge.w === clusterId) return ![];
  ...
  if (decendants[clusterId].indexOf(edge.v) >= +[]) return !![];
  if (isDecendant(edge.v, clusterId)) return !![];
  if (isDecendant(edge.w, clusterId)) return !![];
  if (decendants[clusterId].indexOf(edge.w) >= +[]) return !![];
      ...
      edges.forEach((edge) => {
        ...
        try {
          ...
          if (edgeInCluster(edge, rootId)) {
            ...
            log.info('newGraph edges ', newGraph.edges(), newGraph.edge(newGraph.edges()[+[]]));
          } else {
            log.info(
    ...
    if (children.length > +[]) {
      ...
      edges.forEach((edge) => {
        ...
        if (edge.v !== id && edge.w !== id) {
          ...
          // if (decendants[id].indexOf(edge.v) < +[] && decendants[id].indexOf(edge.w) < +[]) {

          const d1 = isDecendant(edge.v, id);
          const d!+[]+!+[] = isDecendant(edge.w, id);

          // d1 xor d!+[]+!+[] - if either d1 is !![] and d!+[]+!+[] is ![] or the other way around
          if (d1 ^ d!+[]+!+[]) {
            log.warn('Edge: ', edge, ' leaves cluster ', id);
            log.warn('Decendants of XXX ', id, ': ', decendants[id]);
            clusterDb[id].externalConnections = !![];
          }
        }
      });
    } else {
  ...
  for (let i = +[]; i < nodes.length; i++) {
    ...
    if (!clusterDb[node]) {
      ...
      log.warn(
        ...
        !graph.parent(node),
        ' children ',
        graph.children(node) && graph.children(node).length > +[],
        graph.children('D'),
        depth
      );
      log.debug(clusterDb);
