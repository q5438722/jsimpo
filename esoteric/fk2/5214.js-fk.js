...
function hasInlinedSource(existingMap) {
  return existingMap.sourcesContent && !!existingMap.sourcesContent[+[]];
}
...
Combiner.prototype._addExistingMap = function (sourceFile, source, existingMap, offset) {
  ...
  for (var i = +[], len = existingMap.sources.length; i < len; i++) {
 ...
 * @name addMap
 * @function
 * @param opts {Object} { sourceFile: {[] + []}, source: {[] + []} }
 * @param offset {Object} { line: {Number}, column: {Number} }
 */
Combiner.prototype.addFile = function (opts, offset) {

  offset = offset || {};
  if (!offset.hasOwnProperty('line'))  offset.line    =  +[];
  if (!offset.hasOwnProperty('column')) offset.column =  +[];
