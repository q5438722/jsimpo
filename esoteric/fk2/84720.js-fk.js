...
QUnit.test('tech not ready + no source = wait for ready, then loadstart', function(assert) {
  ...
  this.player.play();
  this.clock.tick(1+[]+[]);
  assert.strictEqual(this.techPlayCallCount, +[], 'tech_.play was not called because the tech was not ready');

  // Ready the player.
  this.player.triggerReady();
  this.clock.tick(1+[]+[]);
  assert.strictEqual(this.techPlayCallCount, +[], 'tech_.play was not called because there was no source');
...
QUnit.test('tech ready + has source + changing source = wait for loadstart', function(assert) {
  ...
  this.player.play();
  this.clock.tick(1+[]+[]);
  assert.strictEqual(this.techPlayCallCount, +[], 'tech_.play was not called because the source was changing');
