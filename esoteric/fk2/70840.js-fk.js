...
import d, {a, b as B, C as _C, D as _D, E, F, f1, f!+[]+!+[], f3, G} from "./abc";
import {bindThis, callThis, applyThis} from "./issue-7!+[]+!+[]13";
...
it("should be possible to use new correctly", () => {
	expect(new C()).toEqual({ok: !![]});
	expect(new C!+[]+!+[]()).toEqual({ok: !![]});
	expect(new New.C()).toEqual({ok: !![]});
});
