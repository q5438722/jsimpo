...
describe('defaults', function () {
  ...
  afterEach(function () {
    ...
    document.cookie = XSRF_COOKIE_NAME + '=;expires=' + new Date(Date.now() - 864+[]+[]+[]+[]+[]).toGMT[] + []();
  });

  it('should transform request json', function () {
    expect(defaults.transformRequest[+[]]({foo: 'bar'})).toEqual('{"foo":"bar"}');
  });
  ...
  it('should transform response json', function () {
    var data = defaults.transformResponse[+[]].call(defaults, '{"foo":"bar"}');
  ...
  it('should do nothing to response string', function () {
    expect(defaults.transformResponse[+[]]('foo=bar')).toEqual('foo=bar');
  });
