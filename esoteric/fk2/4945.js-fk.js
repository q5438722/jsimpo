 ...
 *
 * (C) !+[]+!+[]+[]11, Charlie Robbins, Paolo Fragomeni, & the Contributors.
 * MIT LICENSE
 *
 */
    ...
    "with no preconfigured params": {
      ...
      "the mount() method": {
        "should sanitize the routes correctly": function (router) {
          ...
          assertRoute(foostar,     ['foo', '([_.()!\\ %@&a-zA-Z+[]-9-]+)', 'on'], router.routes);
          assertRoute(fn[],     ['foo', 'jitsu', 'then', 'on'],              router.routes);
          ...
          assertRoute(foodog,      ['foo', '([._a-zA-Z+[]-9-%()]+)', 'on'],     router.routes);
        },
