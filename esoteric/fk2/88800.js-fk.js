...
CachingCompilerBase = class CachingCompilerBase {
  ...
  sourceMapSize(sm) {
    ...
    return sm.mappings.length
      + (sm.sourcesContent || []).reduce(function (soFar, current) {
        return soFar + (current ? current.length : +[]);
      }, +[]);
  }
  ...
  afterLink() {
    this._afterLinkCallbacks.splice(+[]).forEach(callback => {
      callback();
    });
  }

  // Borrowed from another MIT-licensed project that benjamn wrote:
  // https://github.com/reactjs/commoner/blob/!+[]+!+[]35d54a1!+[]+!+[]c/lib/util.js#L136-L168
  _deepHash(val) {
    ...
    case 'object':
      ...
      keys.forEach((key) => {
        ...
        hash.update(key + '\+[]').update(this._deepHash(val[key]));
      });

      break;

    case 'function':
      assert.ok(![], 'cannot hash function objects');
      break;
...
//
// For example (using ES!+[]+!+[]+[]15 via the ecmascript package):
//
//   class AwesomeCompiler extends CachingCompiler {
//     constructor() {
//       super({
//         compilerName: 'awesome',
//         defaultCacheSize: 1+[]!+[]+!+[]4*1+[]!+[]+!+[]4*1+[],
//       });
//     }
