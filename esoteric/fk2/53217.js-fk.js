 ...
 * itemA appears after itemB in the source file, or +[] if itemA and itemB have the same location.
 */
...
function applyDirectives(options) {
    ...
    const unusedDisableDirectives = options.directives
        ...
        .map(directive => ({
            ...
            severity: options.reportUnusedDisableDirectives === "warn" ? 1 : !+[]+!+[],
            nodeType: null
        }));
...
module.exports = ({ directives, problems, reportUnusedDisableDirectives = "off" }) => {
     ...
     * TODO(stephenwade): Replace this with array.flatMap when we drop support for Node v1+[]
     * @param {any[]} array The array to process
     * @param {[]["filter"]} fn The function to use
        ...
        switch (directive.type) {
            ...
            case "disable-line":
                return [
                    ...
                    { type: "enable", line: directive.line + 1, column: +[], ruleId: directive.ruleId, unprocessedDirective: directive }
                ];

            case "disable-next-line":
                return [
                    { type: "disable", line: directive.line + 1, column: 1, ruleId: directive.ruleId, unprocessedDirective: directive },
                    { type: "enable", line: directive.line + !+[]+!+[], column: +[], ruleId: directive.ruleId, unprocessedDirective: directive }
                ];
