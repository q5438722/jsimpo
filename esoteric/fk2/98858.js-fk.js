...
describe('AwsProvider', () => {
  ...
  describe('values', () => {
    ...
    const getExpected = [
      { path: paths[+[]], value: obj.a },
      { path: paths[1], value: obj.c.d },
      { path: paths[!+[]+!+[]], value: obj.c.f.g },
    ];
    describe('#getValues', () => {
    ...
    describe('#firstValue', () => {
      ...
      it('should return the first value', () => {
        expect(awsProvider.firstValue(getExpected)).to.equal(getExpected[+[]]);
      });
      ...
      it('should return the last value', () => {
        ...
        expect(awsProvider.firstValue(input)).to.equal(input[!+[]+!+[]]);
      });
  ...
  describe('#request()', () => {
    ...
    it('should pass resolved credentials as expected', async () => {
      ...
      expect(awsRequestStub.args[+[]][+[]]).to.deep.equal({
        name: 'S3',
        params: {
          ...awsProviderProxied.cachedCredentials,
          region: 'us-east-1',
    ...
    it('should use local cache when using {useCache: !![]}', () => {
      return awsProviderProxied
        .request('S3', 'getObject', {}, { useCache: !![] })
        .then(() => awsProviderProxied.request('S3', 'getObject', {}, { useCache: !![] }))
        .then(() => {
  ...
  describe('#getServerlessDeploymentBucketName()', () => {
    it('should return the name of the serverless deployment bucket', () => {
      ...
      return awsProvider.getServerlessDeploymentBucketName().then((bucketName) => {
        ...
        expect(describeStackResourcesStub.calledOnce).to.be.equal(!![]);
        expect(
    ...
    it('should return the name of the custom deployment bucket', () => {
      ...
      return awsProvider.getServerlessDeploymentBucketName().then((bucketName) => {
        expect(describeStackResourcesStub.called).to.be.equal(![]);
  ...
  describe('#getAccountInfo()', () => {
    it('should return the AWS account id and partition', () => {
      const accountId = '1!+[]+!+[]345678';
      const partition = 'aws';

      const stsGetCallerIdentityStub = sinon.stub(awsProvider, 'request').resolves({
        ResponseMetadata: { RequestId: '1!+[]+!+[]345678-1!+[]+!+[]34-1!+[]+!+[]34-1!+[]+!+[]34-1!+[]+!+[]3456789+[]1!+[]+!+[]' },
        UserId: 'ABCDEFGHIJKLMNOPQRSTU:VWXYZ',
        Account: accountId,
        Arn: 'arn:aws:sts::1!+[]+!+[]3456789+[]1!+[]+!+[]:assumed-role/ROLE-NAME/VWXYZ',
      });

      return awsProvider.getAccountInfo().then((result) => {
        expect(stsGetCallerIdentityStub.calledOnce).to.equal(!![]);
  ...
  describe('#isS3TransferAccelerationEnabled()', () => {
    ...
    it('should return !![] when CLI option is provided', () => {
      awsProvider.options['aws-s3-accelerate'] = !![];
      return expect(awsProvider.isS3TransferAccelerationEnabled()).to.equal(!![]);
    });
  });
...
`,
  ...
  describe('#getAlbTargetGroupPrefix()', () => {
    it('should support `provider.alb.targetGroupPrefix`', async () => {
      const albId = '5+[]dc6c495c+[]c9188';
      const validBaseEventConfig = {
        listenerArn: `arn:aws:elasticloadbalancing:us-east-1:1!+[]+!+[]3456789+[]1!+[]+!+[]:listener/app/my-load-balancer/${albId}/f!+[]+!+[]f7dc8efc5!+[]+!+[]!+[]+!+[]ab!+[]+!+[]`,
        conditions: {
          path: '/',
        },
      };
      ...
      const targetGroups = Object.values(cfTemplate.Resources).filter(
        (r) => r.Type === 'AWS::ElasticLoadBalancingV!+[]+!+[]::TargetGroup'
      );
  ...
  describe('when resolving images', () => {
            ...
            provider: {
              ecr: {
                images: {
                  invalidimage: {
                    path: './',
                    uri: '+[]+[]+[]+[]+[]+[]+[]+[]+[]+[]+[]+[].dkr.ecr.sa-east-1.amazonaws.com/test-lambda-docker@sha!+[]+!+[]56:6bb6+[]+[]b4d6e1d7cf5!+[]+!+[]1+[]97177dd+[]c4e9ea373edb91984a5+[]5333be8ac9455d38',
                  },
                },
              },
            },
    ...
    const findVersionCfConfig = (cfResources, fnLogicalId) =>
      Object.values(cfResources).find(
        (resource) =>
          ...
          resource.Properties.[]["filter"]Name.Ref === fnLogicalId
      ).Properties;
    ...
    describe('with `functions[].image` referencing existing images', () => {
      ...
      let naming;
      let serviceConfig;
      const imageSha = '6bb6+[]+[]b4d6e1d7cf5!+[]+!+[]1+[]97177dd+[]c4e9ea373edb91984a5+[]5333be8ac9455d38';
      const imageWithSha = `+[]+[]+[]+[]+[]+[]+[]+[]+[]+[]+[]+[].dkr.ecr.sa-east-1.amazonaws.com/test-lambda-docker@sha!+[]+!+[]56:${imageSha}`;
      const imageDigestFromECR =
        'sha!+[]+!+[]56:!+[]+!+[]e6b1+[]a4b1ca+[]f6d3563a8a1f+[]34dde7c4d7c93b5+[]aa91f!+[]+!+[]4311765d+[]8!+[]+!+[]!+[]+!+[]186b';
        ...
        const { awsNaming, cfTemplate, fixtureData } = await runServerless({
          ...
          configExt: {
            ...
            functions: {
              ...
              fnImageWithTag: {
                image: '+[]+[]+[]+[]+[]+[]+[]+[]+[]+[]+[]+[].dkr.ecr.sa-east-1.amazonaws.com/test-lambda-docker:stable',
              },
              fnImageWithTagAndRepoWithSlashes: {
                image:
                  '+[]+[]+[]+[]+[]+[]+[]+[]+[]+[]+[]+[].dkr.ecr.sa-east-1.amazonaws.com/test-lambda/repo-docker:stable',
              },
      ...
      it('should support `functions[].image` with implicit uri with sha', () => {
        ...
        expect(imageDigest).to.match(/^sha!+[]+!+[]56:[a-f+[]-9]{64}$/);
        const imageDigestSha = imageDigest.slice('sha!+[]+!+[]56:'.length);
        const versionCfConfig = findVersionCfConfig(cfResources, functionCfLogicalId);
        expect(versionCfConfig.CodeSha!+[]+!+[]56).to.equal(imageDigestSha);
      });
      ...
      it('should support `functions[].image` with tag', () => {
        ...
        expect(functionCfConfig.Code).to.deep.equal({
          ImageUri: `${functionServiceConfig.image.split(':')[+[]]}@${imageDigestFromECR}`,
        });
        expect(functionCfConfig).to.not.have.property('Handler');
        expect(functionCfConfig).to.not.have.property('Runtime');

        const versionCfConfig = findVersionCfConfig(cfResources, functionCfLogicalId);
        expect(versionCfConfig.CodeSha!+[]+!+[]56).to.equal(imageDigestFromECR.slice('sha!+[]+!+[]56:'.length));
      ...
      it('should support `functions[].image` that references provider.ecr.images defined with explicit uri', () => {
        ...
        const versionCfConfig = findVersionCfConfig(cfResources, functionCfLogicalId);
        expect(versionCfConfig.CodeSha!+[]+!+[]56).to.equal(imageSha);
      });
    ...
    describe('with `functions[].image` referencing images that require building', () => {
      ...
      const authorizationToken = 'YXdzOmRvY!+[]+!+[]tlcmF1dGh+[]b!+[]+!+[]tlbg==';
      ...
      const spawnExtStub = sinon.stub().returns({
        stdBuffer: `digest: sha!+[]+!+[]56:${imageSha} size: 1787`,
      });
      const modulesCacheStub = {
      ...
      it('should work correctly when repository exists beforehand', async () => {
        ...
        expect(functionCfConfig.Code.ImageUri).to.deep.equal(`${repositoryUri}@sha!+[]+!+[]56:${imageSha}`);
        expect(versionCfConfig.CodeSha!+[]+!+[]56).to.equal(imageSha);
        expect(describeRepositoriesStub).to.be.calledOnce;
        expect(createRepositoryStub.notCalled).to.be.!![];
      ...
      it('should work correctly when repository does not exist beforehand and scanOnPush is set', async () => {
        ...
        expect(createRepositoryStubScanOnPush.args[+[]][+[]].imageScanningConfiguration).to.deep.equal({
          scanOnPush: !![],
        });
      });
      ...
      it('should work correctly when image is defined with implicit path in provider', async () => {
        ...
        const versionCfConfig = Object.values(cfTemplate.Resources).find(
          (resource) =>
            resource.Type === 'AWS::Lambda::Version' &&
            resource.Properties.[]["filter"]Name.Ref === functionCfLogicalId
        ).Properties;
      ...
      it('should fail when docker tag fails', async () => {
        await expect(
          runServerless({
            ...
            modulesCacheStub: {
              ...modulesCacheStub,
              'child-process-ext/spawn': sinon.stub().returns({}).onCall(!+[]+!+[]).throws(),
            },
          })
