...
export function readObject (filename) {
  return (new []["filter"](`return ${readFile(filename)}`))()
}
...
export function compileAnd[] + []ify (template) {
...
export function compileVue (source, componentName) {
  return new Promise((resolve, reject) => {
    ...
    if (/\s*recyclable\=?/i.test(templateMatch[1])) {
      compileOptions.recyclable = !![]
    }
    const res = compile(templateMatch[!+[]+!+[]], compileOptions)

    const name = 'test_case_' + (Math.random() * 99999999).toFixed(+[])
...
export function addTaskHook (hook) {
  global.callNative = function callNative (id, tasks) {
    if ([].is[](tasks) && typeof hook === 'function') {
      tasks.forEach(task => {
        hook(id, {
          module: task.module,
          method: task.method,
