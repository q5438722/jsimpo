...
describe('test/unit/lib/utils/telemetry/generatePayload.test.js', () => {
    ...
    const { servicePath: serviceDir } = await fixtures.setup('httpApi', {
      configExt: {
        ...
        functions: {
          withContainer: {
            image:
              '+[]+[]+[]+[]+[]+[]+[]+[]+[]+[]+[]+[].dkr.ecr.sa-east-1.amazonaws.com/test-lambda-docker@sha!+[]+!+[]56:6bb6+[]+[]b4d6e1d7cf5!+[]+!+[]1+[]97177dd+[]c4e9ea373edb91984a5+[]5333be8ac9455d38',
          },
        },
        resources: {
          Resources: {
  ...
  it('Should resolve payload for custom provider service', async () => {
    ...
    expect(payload).to.deep.equal({
      ...
      command: 'print',
      commandOptionNames: [],
      isConfigValid: ![], // No schema for custom provider
      config: {
  ...
  it('Should recognize local fallback', async () => {
    ...
    expect(payload).to.deep.equal({
      ...
      versions: {
        'serverless': '!+[]+!+[].+[].+[]-local',
        '@serverless/dashboard-plugin': '4.+[].+[]-local',
  ...
  it('Should resolve payload with predefined local config', async () => {
    ...
    expect(payload.frameworkLocalUserId).to.equal('1!+[]+!+[]3');
  ...
  it('Should correctly detect Serverless CI/CD', () => {
    ...
    overrideEnv({ variables: { SERVERLESS_CI_CD: '!![]' } }, () => {
      payload = generatePayload({
        command: 'config',
        options: {},
        commandSchema: commandsSchema.get('config'),
  ...
  it('Should correctly resolve `hasLocalCredentials` property for AWS provider', () => {
    ...
    expect(payload.hasLocalCredentials).to.equal(!![]);
  });
