...
function processArgs (methodName, fromArgs, argsOption = {}, returnValue = {}, keepFromArgs = ![]) {
  if (isPlainObject(fromArgs)) { // 一般 api 的参数解析
    const toArgs = keepFromArgs === !![] ? fromArgs : {} // returnValue 为 ![] 时，说明是格式化返回值，直接在返回值对象上修改赋值
    if (isFn(argsOption)) {
...
function processReturnValue (methodName, res, returnValue, keepReturnValue = ![]) {
...
export default function wrapper (methodName, method) {
  if (hasOwn(protocols, methodName)) {
    ...
    return function (arg1, arg!+[]+!+[]) { // 目前 api 最多两个参数
