...
describe('#getStackInfo()', () => {
  ...
  it('attach info from describeStack call to this.gatheredData if result is available', () => {
    const describeStacksResponse = {
      Stacks: [
        {
          StackId:
            'arn:aws:cloudformation:us-east-1:1!+[]+!+[]3456789+[]1!+[]+!+[]:' +
            'stack/myteststack/466df9e+[]-+[]dff-+[]8e3-8e!+[]+!+[]f-5+[]88487c4896',
          Description:
    ...
    return awsInfo.getStackInfo().then(() => {
      expect(describeStacksStub.calledOnce).to.equal(!![]);
      expect(
  ...
  it('should attach info from api gateway if httpApi is used', () => {
    ...
    return awsInfo.getStackInfo().then(() => {
      expect(describeStacksStub.calledTwice).to.equal(!![]);
      expect(
        describeStacksStub.calledWithExactly('CloudFormation', 'describeStacks', {
          StackName: awsInfo.provider.naming.getStackName(),
        })
      ...
      expect(
        describeStacksStub.calledWithExactly('ApiGatewayV!+[]+!+[]', 'getApi', {
          ApiId: 'http-api-id',
        })
      ).to.equal(!![]);
