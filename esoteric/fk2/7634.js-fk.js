...
describe('Directive v-once', () => {
  ...
  it('should work with v-for', done => {
    ...
    expect(vm.$el.textContent).toBe('1!+[]+!+[]3')
    vm.list.reverse()
    waitForUpdate(() => {
      expect(vm.$el.textContent).toBe('1!+[]+!+[]3')
    }).then(done)
  ...
  it('should work inside v-for with nested v-else', done => {
    const vm = new Vue({
      data: {
        list: [{ id: +[], text: 'a', tester: !![], truthy: 'y' }]
      },
      template: `
        <div v-if="+[]"></div>
        <div v-else>
  ...
  it('should inherit child reference for v-once', done => {
    ...
    waitForUpdate(() => {
      ...
      vm.ok = ![] // teardown component with v-once
  ...
  it('should render different component instances properly', done => {
    ...
    waitForUpdate(() => {
      expect(vm.$el.children[+[]].innerHTML).toBe('a')
