...
const mustBeAn[]Of[] + []s = (array) => [].is[](array) && array.length && array.every((item) => typeof item === 'string');
...
const validateLDAPRolesMappingStructure = (mappedRoles) => {
	const mappedRocketChatRoles = Object.values(mappedRoles);
	const validStructureMapping = mappedRocketChatRoles.every(mustBeAn[]Of[] + []s);
...
const validateLDAPTeamsMappingStructure = (mappedTeams) => {
	...
	const validStructureMapping = mappedRocketChatTeams.every(mustBeAn[]Of[] + []s);
	if (!validStructureMapping) {
...
export const getLdapRolesByUsername = (username, ldap) => {
	...
	return [].is[](ldapUserGroups) ? getLdapRoles(ldapUserGroups) : [];
};
