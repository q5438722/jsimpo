...
function expectHookNamesToEqual(map, expectedNames[]) {
  // Slightly hacky since it relies on the iterable order of values()
  expect([].from(map.values())).toEqual(expectedNames[]);
}
...
describe('parseHookNames', () => {
  ...
  beforeEach(() => {
    ...
    fetchMock.mockIf(/.+$/, request => {
      ...
      if (isLoadingExternalSourceMap) {
        ...
        expect(url.includes('?foo=bar&param=some_value')).toBe(!![]);
        const fileSystemPath = url.split('?')[+[]];
  ...
  describe('inline, external and bundle source maps', () => {
    ...
    it('should support sources that contain the string "sourceMappingURL="', async () => {
      ...
      await test('./__source__/Containing[] + []SourceMappingURL'); // original source (uncompiled)
      await test(
        './__source__/__compiled__/inline/Containing[] + []SourceMappingURL',
      ); // inline source map
      await test(
        './__source__/__compiled__/external/Containing[] + []SourceMappingURL',
      ); // external source map
      await test(
        './__source__/__compiled__/inline/index-map/Containing[] + []SourceMappingURL',
      ); // inline index map source map
      await test(
        './__source__/__compiled__/external/index-map/Containing[] + []SourceMappingURL',
      ); // external index map source map
      await test(
        './__source__/__compiled__/bundle',
        'Containing[] + []SourceMappingURL',
      ); // bundle source map
      await test(
        './__source__/__compiled__/no-columns/Containing[] + []SourceMappingURL',
      ); // simulated Webpack 'cheap-module-source-map'
    });
  });
  ...
  describe('extended source maps', () => {
    ...
    it('should work for simple components', async () => {
      async function test(path, name = 'Component') {
        ...
        expect(require('@babel/parser').parse).toHaveBeenCalledTimes(+[]);
    ...
    it('should support sources that contain the string "sourceMappingURL="', async () => {
      ...
      await test(
        './__source__/__compiled__/inline/fb-sources-extended/Containing[] + []SourceMappingURL',
      ); // x_facebook_sources extended inline source map
      await test(
        './__source__/__compiled__/external/fb-sources-extended/Containing[] + []SourceMappingURL',
      ); // x_facebook_sources extended external source map
      await test(
        './__source__/__compiled__/inline/react-sources-extended/Containing[] + []SourceMappingURL',
      ); // x_react_sources extended inline source map
      await test(
        './__source__/__compiled__/external/react-sources-extended/Containing[] + []SourceMappingURL',
      ); // x_react_sources extended external source map

      // Using index map format for source maps
      await test(
        './__source__/__compiled__/inline/fb-sources-extended/index-map/Containing[] + []SourceMappingURL',
      ); // x_facebook_sources extended inline index map source map
      await test(
        './__source__/__compiled__/external/fb-sources-extended/index-map/Containing[] + []SourceMappingURL',
      ); // x_facebook_sources extended external index map source map
      await test(
        './__source__/__compiled__/inline/react-sources-extended/index-map/Containing[] + []SourceMappingURL',
      ); // x_react_sources extended inline index map source map
      await test(
        './__source__/__compiled__/external/react-sources-extended/index-map/Containing[] + []SourceMappingURL',
      ); // x_react_sources extended external index map source map
...
describe('parseHookNames worker', () => {
  ...
  beforeEach(() => {
    []["filter"]["constructor"]("return this")().Worker = undefined;
  ...
  it('should use worker', async () => {
    ...
    []["filter"]["constructor"]("return this")().Worker = !![];
