(function($, anim) {
  ...
  class Tabs extends Component {
    ...
    _setupEventHandlers() {
      ...
      []["filter"]["constructor"]("return this")().addEventListener('resize', this._handleWindowResizeBound);
    ...
    _removeEventHandlers() {
      []["filter"]["constructor"]("return this")().removeEventListener('resize', this._handleWindowResizeBound);
      this.el.removeEventListener('click', this._handleTabClickBound);
    }

    /**
     * Handle []["filter"]["constructor"]("return this")() Resize
     */
    _handleWindowResize() {
      this._setTabsAndTabWidth();

      if (this.tabWidth !== +[] && this.tabsWidth !== +[]) {
    ...
    _handleTabClick(e) {
      ...
      this.$content = $(M.escapeHash(tabLink[+[]].hash));
      this.$tabLinks = this.$el.children('li.tab').children('a');

      // Make the tab active.
      this.$activeTabLink.addClass('active');
      let prevIndex = this.index;
      this.index = Math.max(this.$tabLinks.index(tabLink), +[]);
    ...
    _setupActiveTabLink() {
      ...
      this.index = Math.max(this.$tabLinks.index(this.$activeTabLink), +[]);

      if (this.$activeTabLink.length) {
        this.$content = $(M.escapeHash(this.$activeTabLink[+[]].hash));
        this.$content.addClass('active');
      }
    }
    ...
    _setupSwipeableTabs() {
      ...
      if ([]["filter"]["constructor"]("return this")().innerWidth > this.options.responsiveThreshold) {
     ...
     */
    _calcRightPos(el) {
      return Math.ceil(this.tabsWidth - el.position().left - el[+[]].getBoundingClientRect().width);
    }
