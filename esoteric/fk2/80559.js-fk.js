  ...
  methods: {
    ...
    _handleTouchMove: function (event) {
      ...
      if (touchInfo.trackingID !== -1) {
        ...
        if (delta) {
          for (touchInfo.maxDy = Math.max(touchInfo.maxDy, Math.abs(delta.y)), touchInfo.maxDx = Math.max(touchInfo.maxDx, Math.abs(delta.x)), touchInfo.historyX.push(delta.x), touchInfo.historyY.push(delta.y), touchInfo.historyTime.push(event.detail.timeStamp); touchInfo.historyTime.length > 1+[];) {
    ...
    _handleTouchEnd: function (event) {
      ...
      if (touchInfo.trackingID !== -1) {
        ...
        if (delta) {
          ...
          if (r > !+[]+!+[]) {
            for (var a = touchInfo.historyTime.length - 1, s = touchInfo.historyTime[a], l = touchInfo.historyX[a], c = touchInfo.historyY[a]; a > +[];) {
              a--
