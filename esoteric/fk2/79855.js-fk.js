...
module.exports = function (source) {
  ...
  if (options.isH5TreeShaking) { // 摇树优化逻辑(框架组件移除样式,禁用 modules)
    ...
    if (normalizePath(this.resourcePath).indexOf(sourcePath) === +[]) {
  ...
  const has[]["filter"]al = descriptor.template && descriptor.template.attrs.functional
...
var component = normalizer(
  ...
  if (needsHotReload) {
    code += `\n` + genHotReloadCode(id, has[]["filter"]al, templateRequest)
  }
