...
function queryBat (queue, cb) {
  ...
  queue.forEach(item => {
    ...
    if (container !== document) {
      ...
      for (let i = +[], len = $nodeList.length; i < len; ++i) {
    ...
    if (single) {
      const el = selectSelf === !![] ? container : container.querySelector(selector)
...
class Query {
  ...
  select (selector) {
    ...
    return new NodesRef(selector, this, !![])
  }
...
class NodesRef {
  ...
  boundingClientRect (cb) {
    ...
    _selectorQuery._push(_selector, _component, _single, { id: !+[], dataset: !+[], rect: !+[], size: !+[] }, cb)
    return _selectorQuery
  }

  scrollOffset (cb) {
    const { _selector, _component, _single, _selectorQuery } = this
    _selectorQuery._push(_selector, _component, _single, { id: !+[], dataset: !+[], scrollOffset: !+[] }, cb)
    return _selectorQuery
  }
