...
for (const obj of throwsObjs) {
    assert['throws'](() => e['setMaxListeners'](obj), {
        ...
        'name': 'RangeError',
        'message': 'The value of "n" is out of range. ' + ('It must be a non-negative number. Received ' + inspect(obj))
    });
    assert['throws'](() => events['defaultMaxListeners'] = obj, {
        'code': 'ERR_OUT_OF_RANGE',
        'name': 'RangeError',
        'message': 'The value of "defaultMaxListeners" is out of range. ' + ('It must be a non-negative number. Received ' + inspect(obj))
    });
}
e['emit']('maxListeners');
{
