...
const snapshotConsoleLogs = function(result) {
  const promptLine = _.chain(console.log.args).map((e) => {
    return e.join(" ");
  }).join("\n").value();
  process.chdir(previousCwd);
  return snapshot(result, stripAnsi(promptLine));
};
describe("lib/cypress", () => {
  ...
  beforeEach(function() {
    ...
    this.expectExitWith = (e) => {
      expect(process.exit).to.be.calledWith(e);
    };
    this.expectExitWithErr = (e, client, dbusItem) => {
      expect(errors.log, "error was logged").to.be.calledWithMatch({
        type : e
      });
      expect(process.exit, "process.exit was called").to.be.calledWith(1);
      const s = errors.log.getCall(0).args[0];
      if (client) {
        expect(s.message, "error text").to.include(client);
      }
      if (dbusItem) {
        expect(s.message, "second error text").to.include(dbusItem);
      }
      return s;
    };
  });
  ...
  context("test browsers", () => {
    ...
    it("allows browser major to be a number", () => {
      ...
      expect(v.isValidBrowserList("browsers", [colix])).to.be.true;
    });
    it("validates returned list", () => {
      return browserUtils.getBrowsers().then((e) => {
        expect(v.isValidBrowserList("browsers", e)).to.be.true;
      });
    });
  });
  context("error handling", function() {
    it("exits if config cannot be parsed", function() {
      return cypress.start(["--config", "xyz"]).then(() => {
        const a = this.expectExitWithErr("COULD_NOT_PARSE_ARGUMENTS");
        snapshot("could not parse config error", stripAnsi(a.message));
      });
    });
    it("exits if env cannot be parsed", function() {
      return cypress.start(["--env", "a123"]).then(() => {
        const a = this.expectExitWithErr("COULD_NOT_PARSE_ARGUMENTS");
        snapshot("could not parse env error", stripAnsi(a.message));
      });
    });
    it("exits if reporter options cannot be parsed", function() {
      return cypress.start(["--reporterOptions", "nonono"]).then(() => {
        const a = this.expectExitWithErr("COULD_NOT_PARSE_ARGUMENTS");
        snapshot("could not parse reporter options error", stripAnsi(a.message));
      });
    });
  });
  ...
  context("--run-project", () => {
    ...
    it("sets --headed false if --headless", function() {
      ...
      return cypress.start([`--run-project=${this.todosPath}`, "--headless"]).then(() => {
        ...
        const button2Component = cypress.startInMode.firstCall.args[1];
        expect(button2Component).to.include({
          headless : true,
          headed : false
        });
      });
    ...
    it("throws an error if both --headed and --headless are true", function() {
      expect(() => {
        return cypress.start([`--run-project=${this.todosPath}`, "--headless", "--headed"]);
    ...
    it("runs project headlessly and exits with exit code 10", function() {
      sinon.stub(runMode, "runSpecs").resolves({
        totalFailed : 10
      });
      return cypress.start([`--run-project=${this.todosPath}`]).then(() => {
    ...
    it("does not generate a project id even if missing one", function() {
      ...
      return user.set({
        ...
        return (new ProjectBase({
          ...
          expect(e.type).to.eq("NO_PROJECT_ID");
        });
      });
    });
    it("does not add project to the global cache", function() {
      return cache.getProjectRoots().then((e) => {
    ...
    it("runs project by relative spec and exits with status 0", function() {
      const e = path.relative(cwd(), this.todosPath);
      return cypress.start([`--run-project=${this.todosPath}`, `--spec=${e}/tests/test2.coffee`]).then(() => {
        expect(browsers.open).to.be.calledWithMatch(ELECTRON_BROWSER, {
    ...
    it("runs project by specific spec with default configuration", function() {
      return cypress.start([`--run-project=${this.idsPath}`, `--spec=${this.idsPath}/cypress/integration/bar.js`, "--config", "port=2020"]).then(() => {
        expect(browsers.open).to.be.calledWithMatch(ELECTRON_BROWSER, {
    ...
    it("runs project by specific absolute spec and exits with status 0", function() {
      return cypress.start([`--run-project=${this.todosPath}`, `--spec=${this.todosPath}/tests/test2.coffee`]).then(() => {
        expect(browsers.open).to.be.calledWithMatch(ELECTRON_BROWSER, {
          url : "http://localhost:8888/__/#/tests/integration/test2.coffee"
        });
        this.expectExitWith(0);
    ...
    it("runs project by limiting spec files via config.testFiles string glob pattern", function() {
      return cypress.start([`--run-project=${this.todosPath}`, `--config=testFiles=${this.todosPath}/tests/test2.coffee`]).then(() => {
        expect(browsers.open).to.be.calledWithMatch(ELECTRON_BROWSER, {
          url : "http://localhost:8888/__/#/tests/integration/test2.coffee"
        });
        this.expectExitWith(0);
    ...
    it("runs project by limiting spec files via config.testFiles as a JSON array of string glob patterns", function() {
      return cypress.start([`--run-project=${this.todosPath}`, '--config=testFiles=["**/test2.coffee","**/test1.js"]']).then(() => {
        expect(browsers.open).to.be.calledWithMatch(ELECTRON_BROWSER, {
          url : "http://localhost:8888/__/#/tests/integration/test2.coffee"
        });
      }).then(() => {
    ...
    it("does not watch settings or plugins in run mode", function() {
      const e = sinon.spy(Watchers.prototype, "watch");
      const listenerNotCall = sinon.spy(Watchers.prototype, "watchTree");
      return cypress.start([`--run-project=${this.pluginConfig}`]).then(() => {
        expect(listenerNotCall).not.to.be.called;
        expect(e).not.to.be.called;
        this.expectExitWith(0);
      });
    });
    it("scaffolds out integration and example specs if they do not exist when not runMode", function() {
      return config.get(this.pristinePath).then((args) => {
        return fs.statAsync(args.integrationFolder).then(() => {
          throw new Error("integrationFolder should not exist!");
        }).catch(() => {
          return cypress.start([`--run-project=${this.pristinePath}`, "--no-run-mode"]);
        }).then(() => {
          return fs.statAsync(args.integrationFolder);
        }).then(() => {
          return Promise.join(fs.statAsync(path.join(args.integrationFolder, "1-getting-started", "todo.spec.js")), fs.statAsync(path.join(args.integrationFolder, "2-advanced-examples", "actions.spec.js")), fs.statAsync(path.join(args.integrationFolder, "2-advanced-examples", "files.spec.js")), fs.statAsync(path.join(args.integrationFolder, "2-advanced-examples", "viewport.spec.js")));
        });
      });
    });
    it("does not scaffold when headless and exits with error when no existing project", function() {
      const init = function(e, t) {
        if (!e.isRejected()) {
          throw new Error(`File or folder was scaffolded at index: ${t}`);
        }
        expect(e.reason()).to.have.property("code", "ENOENT");
      };
      return Promise.all([fs.statAsync(path.join(this.pristinePath, "cypress")).reflect(), fs.statAsync(path.join(this.pristinePath, "cypress.json")).reflect()]).each(init).then(() => {
        return cypress.start([`--run-project=${this.pristinePath}`]);
      }).then(() => {
        return Promise.all([fs.statAsync(path.join(this.pristinePath, "cypress")).reflect(), fs.statAsync(path.join(this.pristinePath, "cypress.json")).reflect()]);
      }).each(init).then(() => {
    ...
    it("scaffolds out fixtures + files if they do not exist", function() {
      return config.get(this.pristinePath).then((options) => {
        return fs.statAsync(options.fixturesFolder).then(() => {
          throw new Error("fixturesFolder should not exist!");
        }).catch(() => {
          return cypress.start([`--run-project=${this.pristinePath}`, "--no-run-mode"]);
        }).then(() => {
          return fs.statAsync(options.fixturesFolder);
        }).then(() => {
          return fs.statAsync(path.join(options.fixturesFolder, "example.json"));
        });
      });
    });
    it("scaffolds out support + files if they do not exist", function() {
      const uploadsTempPath = path.join(this.pristinePath, "cypress/support");
      return config.get(this.pristinePath).then(() => {
        return fs.statAsync(uploadsTempPath).then(() => {
          ...
          return fs.statAsync(uploadsTempPath);
        }).then(() => {
          return fs.statAsync(path.join(uploadsTempPath, "index.js"));
        }).then(() => {
          return fs.statAsync(path.join(uploadsTempPath, "commands.js"));
        });
      });
    });
    it("removes fixtures when they exist and fixturesFolder is false", function(saveNotifs) {
      config.get(this.idsPath).then((cfg) => {
        this.cfg = cfg;
        return fs.statAsync(this.cfg.fixturesFolder);
      }).then(() => {
        return settings.read(this.idsPath);
      }).then((options) => {
        options.fixturesFolder = false;
        return settings.write(this.idsPath, options);
      }).then(() => {
    ...
    it("can change the reporter with cypress.json", function() {
      sinon.spy(Reporter, "create");
      return config.get(this.idsPath).then((cfg) => {
        this.cfg = cfg;
        return settings.read(this.idsPath);
      }).then((e) => {
        e.reporter = "dot";
        return settings.write(this.idsPath, e);
      }).then(() => {
        return cypress.start([`--run-project=${this.idsPath}`]);
      }).then(() => {
    ...
    it("logs warning when removing old browser profiles fails", function() {
      ...
      sinon.stub(browsers, "removeOldProfiles").rejects(error);
      return cypress.start([`--run-project=${this.todosPath}`]).then(() => {
        expect(errors.warning).to.be.calledWith("CANNOT_REMOVE_OLD_BROWSER_PROFILES", error.stack);
        expect(console.log).to.be.calledWithMatch("Warning: We failed to remove old browser profiles from previous runs.");
        expect(console.log).to.be.calledWithMatch(error.message);
      });
    });
    ...
    it("logs error when supportFile doesn't exist", function() {
      return settings.write(this.idsPath, {
    ...
    it("logs error when browser cannot be found", function() {
      ...
      return cypress.start([`--run-project=${this.idsPath}`, "--browser=foo"]).then(() => {
        ...
        const e = errors.log.args;
        const button2Component = _.find(e, (searchDefinition) => {
          return _.find(searchDefinition, (e) => {
            return e.message && e.message.includes("Browser: 'foo' was not found on your system or is not supported by Cypress.");
          });
        });
        expect(button2Component, "foo should not be found").to.be.ok;
        const localBosco = _.find(e, (searchDefinition) => {
          return _.find(searchDefinition, (e) => {
            return e.message && e.message.includes("Cypress supports the following browsers:");
          });
        });
        expect(localBosco, "supported browsers should be listed").to.be.ok;
        const s = _.find(e, (searchDefinition) => {
          return _.find(searchDefinition, (e) => {
            return e.message && e.message.includes("Available browsers found on your system are:\n- chrome\n- chromium\n- chrome:canary\n- electron");
          });
        });
        expect(s, "browser names should be listed").to.be.ok;
      });
    });
    describe("no specs found", function() {
      ...
      it("logs error and exits when spec absolute file was specified and does not exist", function() {
        return cypress.start([`--run-project=${this.todosPath}`, `--spec=${this.todosPath}/tests/path/to/spec`]).then(() => {
          this.expectExitWithErr("NO_SPECS_FOUND", "tests/path/to/spec");
          this.expectExitWithErr("NO_SPECS_FOUND", this.todosPath);
        });
      });
      it("logs error and exits when no specs were found at all", function() {
        return cypress.start([`--run-project=${this.todosPath}`, "--config=integrationFolder=cypress/specs"]).then(() => {
    ...
    it("logs error and exits when project has cypress.json syntax error", function() {
      return fs.writeFileAsync(`${this.todosPath}/cypress.json`, "{'foo': 'bar}").then(() => {
        return cypress.start([`--run-project=${this.todosPath}`]);
      }).then(() => {
    ...
    it("logs error and exits when project has cypress.env.json syntax error", function() {
      return fs.writeFileAsync(`${this.todosPath}/cypress.env.json`, "{'foo': 'bar}").then(() => {
        return cypress.start([`--run-project=${this.todosPath}`]);
      }).then(() => {
        this.expectExitWithErr("ERROR_READING_FILE", this.todosPath);
      });
    ...
    it("logs error and exits when project has invalid cypress.json values", function() {
      return settings.write(this.todosPath, {
    ...
    pipelets.forEach(function(e) {
      it(`logs error and exits when using an old configuration option: ${e.old}`, function() {
        return cypress.start([`--run-project=${this.todosPath}`, `--config=${e.old}=''`]).then(() => {
          this.expectExitWithErr("RENAMED_CONFIG_OPTION", e.old);
          this.expectExitWithErr("RENAMED_CONFIG_OPTION", e.new);
        });
      });
    });
    it("logs error and exits when project folder has read permissions only and cannot write cypress.json", function() {
      ...
      const file = path.resolve("./permissions");
      const reIndexFile = path.join(file, "cypress.json");
      return fs.outputFileAsync(reIndexFile, "{}").then(() => {
        return fs.chmodAsync(file, "555");
      }).then(() => {
        return cypress.start([`--run-project=${file}`]);
      }).then(() => {
        return fs.chmodAsync(file, "777");
      }).then(() => {
        return fs.removeAsync(file);
      }).then(() => {
        this.expectExitWithErr("ERROR_READING_FILE", path.join(file, "cypress.json"));
      });
    });
    ...
    describe("state", () => {
      beforeEach(function() {
        return appData.remove().then(() => {
          ...
          this.statePath = appData.projectsPath(e);
        });
      });
    ...
    describe("config overrides", () => {
      it("can override default values", function() {
        return cypress.start([`--run-project=${this.todosPath}`, "--config=requestTimeout=1234,videoCompression=false"]).then(() => {
          const {
            cfg : cfg
      ...
      it("can override values in plugins", function() {
        ...
        return cypress.start([`--run-project=${this.pluginConfig}`, "--config=requestTimeout=1234,videoCompression=false", "--env=foo=foo,bar=bar"]).then(() => {
          const {
            cfg : self
          } = openProject.getProject();
          expect(self.videoCompression).to.eq(20);
          expect(self.defaultCommandTimeout).to.eq(500);
          expect(self.env).to.deep.eq({
            foo : "bar",
            bar : "bar"
          });
          expect(self.resolved.videoCompression).to.deep.eq({
            value : 20,
            from : "plugin"
          });
          expect(self.resolved.requestTimeout).to.deep.eq({
            value : 1234,
            from : "cli"
          });
          expect(self.resolved.env.foo).to.deep.eq({
            value : "bar",
            from : "plugin"
          });
          expect(self.resolved.env.bar).to.deep.eq({
            value : "bar",
            from : "cli"
          });
          this.expectExitWith(0);
    ...
    describe("plugins", () => {
      beforeEach(() => {
        ...
        self.maximize = sinon.stub;
        self.setSize = sinon.stub;
        sinon.stub(launch, "launch").resolves(self);
        sinon.stub(Windows, "create").returns(self);
      });
      context("before:browser:launch", () => {
        it("chrome", function() {
          const fetch500Response = {
          ...
          sinon.stub(chromeBrowser, "_connectToChromeRemoteInterface").resolves(fetch500Response);
          ...
          return cypress.start([`--run-project=${this.pluginBrowser}`, "--browser=chrome"]).then(() => {
            const {
              args : e
            } = launch.launch.firstCall;
            const titleComponents = _.find(TYPICAL_BROWSERS, {
              name : "chrome"
            });
            const adminInx = R.merge(titleComponents, {
              isHeadless : true,
              isHeaded : false
            });
            expect(e[0], "found and used Chrome").to.deep.eq(adminInx);
            const charListNotLatin = e[2];
            expect(charListNotLatin.slice(0, 4), "first 4 custom launch arguments to Chrome").to.deep.eq(["chrome", "foo", "bar", "baz"]);
            this.expectExitWith(0);
    ...
    describe("--port", () => {
      beforeEach(() => {
        return runMode.listenForProjectEnd.resolves({
          stats : {
            failures : 0
          }
        });
      ...
      it("can change the default port to 5544", function() {
        const button2Component = sinon.spy(http.Server.prototype, "listen");
        const localBosco = sinon.spy(ServerE2E.prototype, "open");
        return cypress.start([`--run-project=${this.todosPath}`, "--port=5544"]).then(() => {
          expect(openProject.getProject().cfg.port).to.eq(5544);
          expect(button2Component).to.be.calledWith(5544);
          expect(localBosco).to.be.calledWithMatch({
            port : 5544
          });
          this.expectExitWith(0);
        });
      ...
      it("logs error and exits when port is in use", async function() {
        ...
        return server.listenAsync(5544, "127.0.0.1").then(() => {
    ...
    describe("--env", () => {
      ...
      it("can set specific environment variables", function() {
        return cypress.start([`--run-project=${this.todosPath}`, "--video=false", "--env", "version=0.12.1,foo=bar,host=http://localhost:8888,baz=quux=dolor"]).then(() => {
      ...
      it("parses environment variables with empty values", function() {
        return cypress.start([`--run-project=${this.todosPath}`, "--video=false", "--env=FOO=,BAR=,BAZ=ipsum"]).then(() => {
          expect(openProject.getProject().cfg.env).to.deep.eq({
            FOO : "",
            BAR : "",
            BAZ : "ipsum"
    ...
    describe("--config-file", () => {
      it("false does not require cypress.json to run", function() {
        return fs.statAsync(path.join(this.pristinePath, "cypress.json")).then(() => {
          ...
          return cypress.start([`--run-project=${this.pristinePath}`, "--no-run-mode", "--config-file", "false"]).then(() => {
            this.expectExitWith(0);
          });
        });
      });
      it("with a custom config file fails when it doesn't exist", function() {
        ...
        return fs.statAsync(path.join(this.todosPath, this.filename)).then(() => {
          throw new Error(`${this.filename} should not exist`);
        }).catch({
          code : "ENOENT"
        }, () => {
          return cypress.start([`--run-project=${this.todosPath}`, "--no-run-mode", "--config-file", this.filename]).then(() => {
  ...
  context("--record", () => {
    beforeEach(function() {
      ...
      return Promise.all([user.set({}), getId(this.todosPath).then((id) => {
        this.projectId = id;
      })]);
    });
    it("uses process.env.CYPRESS_PROJECT_ID", function() {
      sinon.stub(env, "get").withArgs("CYPRESS_PROJECT_ID").returns(this.projectId);
      return cypress.start(["--cwd=/foo/bar", `--run-project=${this.noScaffolding}`, "--record", "--key=token-123"]).then(() => {
    ...
    it("uses process.env.CYPRESS_RECORD_KEY", function() {
      sinon.stub(env, "get").withArgs("CYPRESS_PROJECT_ID").returns("foo-project-123").withArgs("CYPRESS_RECORD_KEY").returns("token");
    ...
    it("errors and exits when using --group but ciBuildId could not be generated", function() {
      ...
      return cypress.start([`--run-project=${this.recordPath}`, "--record", "--key=token-123", "--group=e2e-tests"]).then(() => {
    ...
    it("errors and exits when using --parallel but ciBuildId could not be generated", function() {
      sinon.stub(ciProvider, "provider").returns(null);
      return cypress.start([`--run-project=${this.recordPath}`, "--record", "--key=token-123", "--parallel"]).then(() => {
        this.expectExitWithErr("INDETERMINATE_CI_BUILD_ID");
    ...
    it("errors and exits when using --parallel and --group but ciBuildId could not be generated", function() {
      sinon.stub(ciProvider, "provider").returns(null);
      return cypress.start([`--run-project=${this.recordPath}`, "--record", "--key=token-123", "--group=e2e-tests-chrome", "--parallel"]).then(() => {
        this.expectExitWithErr("INDETERMINATE_CI_BUILD_ID");
    ...
    it("errors and exits when using --ci-build-id with no group or parallelization", function() {
      return cypress.start([`--run-project=${this.recordPath}`, "--record", "--key=token-123", "--ci-build-id=ciBuildId123"]).then(() => {
    ...
    it("errors and exits when using --group and --parallel without recording", function() {
      return cypress.start([`--run-project=${this.recordPath}`, "--tag=nightly", "--group=electron-smoke-tests", "--parallel"]).then(() => {
        this.expectExitWithErr("RECORD_PARAMS_WITHOUT_RECORDING");
    ...
    it("errors and exits when group name is not unique and explicitly passed ciBuildId", function() {
      ...
      return cypress.start([`--run-project=${this.recordPath}`, "--record", "--key=token-123", "--group=electron-smoke-tests", "--ciBuildId=ciBuildId123"]).then(() => {
    ...
    it("errors and exits when parallel group params are different", function() {
      ...
      api.createRun.rejects(err);
      return cypress.start([`--run-project=${this.recordPath}`, "--record", "--key=token-123", "--parallel", "--group=electron-smoke-tests", "--ciBuildId=ciBuildId123"]).then(() => {
    ...
    it("errors and exits when parallel is required", function() {
      ...
      api.createRun.rejects(err);
      return cypress.start([`--run-project=${this.recordPath}`, "--record", "--key=token-123", "--parallel", "--tag=nightly", "--group=electron-smoke-tests", "--ciBuildId=ciBuildId123"]).then(() => {
    ...
    it("errors and exits when run is already complete", function() {
      ...
      api.createRun.rejects(err);
      return cypress.start([`--run-project=${this.recordPath}`, "--record", "--key=token-123", "--tag=nightly", "--group=electron-smoke-tests", "--ciBuildId=ciBuildId123"]).then(() => {
  ...
  context("interactive", () => {
    ...
    it("passes filtered options to Project#open and sets cli config", function() {
      const button2Component = sinon.stub(ServerE2E.prototype, "open").resolves([]);
      ...
      return user.set({
        ...
        return settings.write(this.todosPath, e);
      }).then(() => {
        return cypress.start(["--port=2121", "--config", "pageLoadTimeout=1000", "--foo=bar", "--env=baz=baz"]);
      }).then(() => {
        const e = Events.start.firstCall.args[0];
        return Events.handleEvent(e, {}, {}, 123, "open:project", this.todosPath);
      }).then(() => {
        const e = openProject.getProject().options;
        expect(e.port).to.eq(2121);
        expect(e.pageLoadTimeout).to.eq(1E3);
        expect(e.report).to.eq(false);
        expect(e.env).to.eql({
          baz : "baz"
        });
        expect(button2Component).to.be.called;
        const config = button2Component.getCall(0).args[0];
        expect(config.fileServerFolder).to.eq(path.join(this.todosPath, "foo"));
        expect(config.pageLoadTimeout).to.eq(1E3);
        expect(config.port).to.eq(2121);
        expect(config.baseUrl).to.eq("http://localhost");
        expect(config.watchForFileChanges).to.be.false;
        expect(config.responseTimeout).to.eq(5555);
        expect(config.env.baz).to.eq("baz");
        expect(config.env).not.to.have.property("fileServerFolder");
        expect(config.env).not.to.have.property("port");
        expect(config.env).not.to.have.property("BASE_URL");
        expect(config.env).not.to.have.property("watchForFileChanges");
        expect(config.env).not.to.have.property("responseTimeout");
        expect(config.resolved.fileServerFolder).to.deep.eq({
          value : "foo",
          from : "env"
        });
        expect(config.resolved.pageLoadTimeout).to.deep.eq({
          value : 1E3,
          from : "cli"
        });
        expect(config.resolved.port).to.deep.eq({
          value : 2121,
          from : "cli"
        });
        expect(config.resolved.baseUrl).to.deep.eq({
          value : "http://localhost",
          from : "env"
        });
        expect(config.resolved.watchForFileChanges).to.deep.eq({
          value : false,
          from : "env"
        });
        expect(config.resolved.responseTimeout).to.deep.eq({
          value : 5555,
          from : "env"
        });
        expect(config.resolved.env.baz).to.deep.eq({
          value : "baz",
          from : "cli"
        });
      });
    ...
    it("sends warning when baseUrl cannot be verified", function() {
      ...
      sinon.stub(ServerE2E.prototype, "open").resolves([2121, model2]);
      return cypress.start(["--port=2121", "--config", "pageLoadTimeout=1000", "--foo=bar", "--env=baz=baz"]).then(() => {
        const e = Events.start.firstCall.args[0];
        Events.handleEvent(e, eventInfo, event, 123, "on:project:warning");
        return Events.handleEvent(e, eventInfo, event, 123, "open:project", this.todosPath);
      }).then(() => {
        expect(event.sender.send.withArgs("response").firstCall.args[1].data).to.eql(model2);
      });
    });
    describe("--config-file", () => {
      beforeEach(function() {
      ...
      it("reads config from a custom config file", function() {
        ...
        return cypress.start([`--config-file=${this.filename}`]).then(() => {
          const e = Events.start.firstCall.args[0];
          return Events.handleEvent(e, {}, {}, 123, "open:project", this.pristinePath);
        }).then(() => {
          expect(this.open).to.be.called;
          const e = this.open.getCall(0).args[0];
          expect(e.env.foo).to.equal("bar");
          expect(e.port).to.equal(2020);
        });
      });
      it("creates custom config file if it does not exist", function() {
        return cypress.start([`--config-file=${this.filename}`]).then(() => {
          ...
          return fs.readJsonAsync(path.join(this.pristinePath, this.filename)).then((e) => {
            expect(e, "json file is empty").to.deep.equal({});
          });
        });
      });
    });
