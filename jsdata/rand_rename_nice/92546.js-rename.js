...
describe("babel 7 smoke test", function() {
  it("should be able to extend a class with native name", function() {
    ...
    class MyClass extends Internal {
      constructor() {
        super();
      }
    }
    expect((new MyClass).foo).toBe("PASS");
    expect(e).toBe(true);
  });
});
describe("Blob", function() {
  it("regression caused by circular dep && babel 7", function() {
    const link_provider = new Blob([], {
      type : "image/jpeg"
    });
    expect(link_provider).toBeInstanceOf(Blob);
  });
});
describe("File", function() {
  it("should create empty file", () => {
    ...
    expect(e.data.size).toBe(0);
    expect(e.size).toBe(0);
    expect(e.type).toBe("");
    expect(e.name).toBe("test.jpg");
    expect(e.lastModified).toEqual(expect.any(Number));
  });
  ...
  it("should create empty file with lastModified", () => {
    const groupBefore = new File([], "test.jpg", {
      lastModified : 1337
    });
    expect(groupBefore.lastModified).toBe(1337);
  });
