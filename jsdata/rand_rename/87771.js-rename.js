import selftest from"../tool-testing/selftest.js";import{parse,markBottom}from"../utils/parse-stack";import _ from"underscore";import Fiber from"fibers";import Future from"fibers/future";import files from"../fs/files";selftest.define("parse-stack - parse stack traces without fibers",()=>{const e=new Error;const s=parse(e).outsideFiber;const t=files.convertToStandardPath(s[0].file);selftest.expectEqual(_.last(t.split("/")),"parse-stack-test.js");const o=files.convertToStandardPath(_.last(s).file);selftest.expectEqual(_.last(o.split("/")),"selftest.js");markBottom(()=>{const e=new Error;const{outsideFiber:s,insideFiber:t}=parse(e);selftest.expectEqual(t,undefined);selftest.expectEqual(s.length,1);const o=files.convertToStandardPath(s[0].file);selftest.expectEqual(_.last(o.split("/")),"parse-stack-test.js")})()});selftest.define("parse-stack - parse stack traces with fibers",()=>{const e=parse({stack:exampleStackTrace});selftest.expectEqual(e.insideFiber[0].file,"template-compiler.js");selftest.expectEqual(_.last(e.outsideFiber).file,"template-compiler.js")});const exampleStackTrace=`TypeError: object is not a function
