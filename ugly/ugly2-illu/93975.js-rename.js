...
var { fs } = require(`${root}lib/util/fs`);
...
describe("lib/util/file", () => {
  beforeEach(function () {
    this.dir = path.join(os.tmpdir(), "cypress", "file_spec");this.path = path.join(this.dir, "file.json");return fs.removeAsync(this.dir).catch(() => {});
    ...
    sinon.spy(lockFile, "unlockSync");sinon.stub(exit, "ensure");new FileUtil({ path: this.path });exit.ensure.yield();expect(lockFile.unlockSync).to.be.called;
  });context("#transaction", () => {
    beforeEach(function () {
      ...
      return Promise.all([this.fileUtil.transaction(t => {
        return t.get("items", []).then(e => {
          return t.set("items", e.concat("foo"));
        });
      }), this.fileUtil.transaction(t => {
        return t.get("items", []).then(e => {
          return t.set("items", e.concat("bar"));
        });
      }), this.fileUtil.transaction(t => {
        return t.get("items", []).then(e => {
          return t.set("items", e.concat("baz"));
        });
      })]).then(() => {
        return this.fileUtil.transaction(e => {
          return e.get("items").then(e => {
            expect(e).to.eql(["foo", "bar", "baz"]);
          });
        });
      });
    });
    ...
    beforeEach(function () {
      ...
      return this.fileUtil.get().then(e => {
        expect(e).to.eql({});
      });
      ...
      return fs.ensureDirAsync(this.dir).then(() => {
        ...
        sinon.stub(lockFile, "lockAsync").rejects({ name: "", message: "", code: "EEXIST" });return this.fileUtil.get();
      }).then(e => {
        expect(e).to.eql({});
      });
      ...
      sinon.stub(fs, "readJsonAsync").resolves({});return Promise.all([this.fileUtil.get(), this.fileUtil.get(), this.fileUtil.get()]).then(() => {
        expect(fs.readJsonAsync).to.be.calledOnce;
      });
    });it("locks file while reading", function () {
      sinon.spy(lockFile, "lockAsync");return this.fileUtil.get().then(() => {
        expect(lockFile.lockAsync).to.be.called;
      });
    });it("unlocks file when finished reading", function () {
      sinon.spy(lockFile, "unlockAsync");return this.fileUtil.get().then(() => {
        expect(lockFile.unlockAsync).to.be.called;
      });
    });it("unlocks file even if reading fails", function () {
      sinon.spy(lockFile, "unlockAsync");sinon.stub(fs, "readJsonAsync").rejects(new Error("fail!"));return this.fileUtil.get().catch(() => {
        expect(lockFile.unlockAsync).to.be.called;
      });
    });it("times out and carries on if unlocking times out", function () {
      sinon.stub(lockFile, "lockAsync").resolves();sinon.stub(lockFile, "unlockAsync").callsFake(() => {
        return Promise.delay(1e9);
      });sinon.stub(fs, "readJsonAsync").resolves({});sinon.stub(env, "get").withArgs("FILE_UNLOCK_TIMEOUT").returns(100);return this.fileUtil.get();
    });
  });context("#set", () => {
    beforeEach(function () {
      this.fileUtil = new FileUtil({ path: this.path });
      ...
      }).to.throw("Expected `key` to be of type `string` or `object`, got `number`");expect(() => {
      ...
      return this.fileUtil.set("foo.baz", "bar").then(() => {
        ...
        expect(e).to.eql({ foo: { baz: "bar" } });
      });
    });it("sets values for object", function () {
      return this.fileUtil.set({ foo: "bar", baz: { qux: "lolz" } }).then(() => {
        return this.fileUtil.get();
      }).then(e => {
        expect(e).to.eql({ foo: "bar", baz: { qux: "lolz" } });
      });
      ...
      return this.fileUtil.set("foo", "bar").then(() => {
        ...
        expect(e).to.eql({ foo: "bar", baz: "qux" });
      });
    });it("updates file on disk", function () {
      return this.fileUtil.set("foo", "bar").then(() => {
        return fs.readFileAsync(path.join(this.dir, "file.json"), "utf8");
      }).then(e => {
        expect(JSON.parse(e)).to.eql({ foo: "bar" });
      });
    });it("locks file while writing", function () {
      sinon.spy(lockFile, "lockAsync");return this.fileUtil.set("foo", "bar").then(() => {
        expect(lockFile.lockAsync).to.be.called;
      });
    });it("unlocks file when finished writing", function () {
      sinon.spy(lockFile, "unlockAsync");return this.fileUtil.set("foo", "bar").then(() => {
        expect(lockFile.unlockAsync).to.be.called;
      });
    });it("unlocks file even if writing fails", function () {
      sinon.spy(lockFile, "unlockAsync");sinon.stub(fs, "outputJsonAsync").rejects(new Error("fail!"));return this.fileUtil.set("foo", "bar").catch(() => {
        expect(lockFile.unlockAsync).to.be.called;
      });
    });
  });context("#remove", () => {
    beforeEach(function () {
      ...
      sinon.spy(lockFile, "lockAsync");return this.fileUtil.remove().then(() => {
        expect(lockFile.lockAsync).to.be.called;
      });
    });it("unlocks file when finished removing", function () {
      sinon.spy(lockFile, "unlockAsync");return this.fileUtil.remove().then(() => {
        expect(lockFile.unlockAsync).to.be.called;
      });
    });it("unlocks file even if removing fails", function () {
      sinon.spy(lockFile, "unlockAsync");sinon.stub(fs, "removeAsync").rejects(new Error("fail!"));return this.fileUtil.remove().then(() => {
        throw new Error("should have caught!");
      }).catch(e => {
        expect(e.message).to.eq("fail!");expect(lockFile.unlockAsync).to.be.called;
      });
    });
  });
});
